...
# encoding=utf8
'''#数据库的路径'''
DATABASE_PATHNAME = "/Users/yangjie/mywork/workspace/mypython/com/office/stitch/omim1011.db"
'''启动的进程数'''
PROCESS_NUM=8
'''#谷歌浏览器驱动的路径'''
...
def doSomething(driver, keyword, tableId, pfi):
    url="http://omim.org/entry/"+keyword
    print url
    driver.get(url)
    try:
    ...
    updateStatus(STATUS_SUCCESS, tableId)
    addResult([[keyword,description,clinicalFeatures]])
'''
以下为框架代码，不需要修改
'''
