    ...
    while True:
        try:
            if driver.current_url == URL:
                raiseOnError = True;
                retry(lambda:driver.find_element_by_id("term").clear(), times=10, allowError=True, raiseOnError=raiseOnError)
                retry(lambda:driver.find_element_by_id("term").send_keys(keyword), times=10, allowError=True, raiseOnError=raiseOnError)
                retry(lambda:driver.find_element_by_id("search").click(), times=10, allowError=True, raiseOnError=raiseOnError)
                time.sleep(0.1)
                isFind = True
...
def getAndUpdateNextKey():
    ...
    try:
        ...
        if result:
            tableId = result[0][0]
            update_sql = 'UPDATE keys set status=1 where id=%d limit 1;' % tableId
...
def updateStatus(status, tableId=None):
    ...
    try:
        ...
        cu = conn.cursor()
        if tableId:
            update_sql = 'UPDATE keys set status=%d where id=%d limit 1;' % (status, tableId)
        else:
