...
def doSomething(driver, keyword, tableId, pfi):
    try:
        ...
        while limitTime > 0:
            try:
                limitTime-=1;
                text = driver.find_element_by_xpath("//div[@class='output-bd']/p[@class='ordinary-output target-output clearfix']/span").text.strip()
                if pfi.lastValue != keyword and text == pfi.lastText:
                    continue
